...
-   **Impersonation Password/Subscription Fix (Dec 2025):** Fixed critical bug where changing password or cancelling subscription while impersonating a user would affect the admin's account instead of the impersonated user. Both `/api/me/reset-password` and `/api/me/cancel-subscription` endpoints now use `getEffectiveUserId()` to correctly target the impersonated user. Audit logs preserve the real admin ID for accountability while tracking the affected user ID.
-   **Bulk Send Impersonation Fix (Dec 2025):** Fixed critical bug where "Use One Message for All" (send-uniform) and regular phonebook bulk send would fail during impersonation. Both `/api/phonebooks/:id/send` and `/api/phonebooks/:id/send-uniform` endpoints now use `getEffectiveUserId()` for phonebook ownership verification, channel ownership verification, running job checks, and job creation. This ensures admins can properly test bulk sending features while impersonating users.
-   **Bulk Job Management System (Dec 2025):** Comprehensive bulk messaging job control system with the following features:
    - **One-at-a-time enforcement:** Users can only have one active bulk campaign (PENDING, QUEUED, or PROCESSING) at a time. New bulk send attempts return 409 Conflict with clear messaging.
    - **Job status PAUSED:** Added PAUSED status to job enum for paused campaigns. Schema updated with `db:push`.
    - **Stop/Resume/Delete controls:** Outbox UI now includes job control buttons. Stop pauses processing (status=PAUSED), Resume restarts processing (status=PROCESSING), Delete removes job and all associated messages.
    - **Daily limit enforcement during processing:** `processBulkJob` now checks daily bulk message limits during message processing (not just at job creation). Jobs exceeding limits are automatically paused with user notification.
    - **Enhanced error handling:** Send page shows distinct toast notifications for "Campaign Already Running" (409 errors) and "Daily Limit Reached" scenarios.
    - **All bulk endpoints protected:** `/api/messages/bulk`, `/api/phonebooks/:id/send`, and `/api/phonebooks/:id/send-uniform` all validate for running jobs before creating new ones.
    - **Button formatting fix:** Bulk send now correctly uses `transformedButtons` with proper "title" field for WHAPI API compatibility instead of original `buttons` array.
