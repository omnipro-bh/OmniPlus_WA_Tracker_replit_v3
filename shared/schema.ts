...
export const jobStatusEnum = pgEnum("job_status", ["QUEUED", "PROCESSING", "PENDING", "SENT", "DELIVERED", "READ", "FAILED", "PARTIAL", "COMPLETED", "PAUSED"]);
...
export const bookingServices = pgTable("booking_services", {
  ...
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  slotLabels: text("slot_labels").array(), // Custom labels for time slots (e.g., weekdays or Morning/Evening/Night)
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
